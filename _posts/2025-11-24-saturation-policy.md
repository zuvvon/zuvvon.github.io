...
스레드 풀은 일정 수준의 부하까지는 빠르게 작업을 소화하지만,  
**maxPoolSize까지 스레드가 증가하고 workQueue까지 가득 찬 순간**에는 더 이상 작업을 받을 수 없다.  
이 상태를 *포화(Saturation)*라고 하고, Java는 이때 실행할 행동을 **RejectedExecutionHandler**, 즉 *포화 정책*으로 정의한다.  
...
**스레드 풀(ThreadPoolExecutor)이 더 이상 작업을 처리할 수 없을 때 선택하는 대응 전략**을 의미한다.
